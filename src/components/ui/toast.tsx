...
const toastVariants = {
  default: "glass-panel-strong border-[var(--glass-border)]",
  success: "glass-panel-strong border-[var(--color-success)]",
  error: "glass-panel-strong border-[var(--color-danger)]",
  warning: "glass-panel-strong border-[var(--color-warning)]",
  info: "glass-panel-strong border-[var(--accent-cyan)]",
};
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-md border border-[var(--glass-border)] bg-transparent px-3 text-sm font-medium transition-colors hover:bg-[var(--glass-bg-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-primary)] focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-[var(--text-muted)] opacity-0 transition-opacity hover:text-[var(--text-secondary)] focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100",
      className
    )}
    toast-close=""
    {...props}
